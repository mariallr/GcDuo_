...
#' @export
#'
duoIDs <- function(DuoResults, lib, match_cutoff = 0.7, RI = NULL, RIrange = 30)
{
  require(dplyr)
  require(Spectra)
  ...
  # spd_lib <- DataFrame(msLevel = rep(2L, nrow(lib_matrix)),
  #                         id = row.names(lib_matrix))
  # spd_lib$mz <- list(GcDuoObject$mz)
  # spd_lib$intensity <- as.list(as.data.frame(t(lib_matrix)))
  #
  # sps_lib <- Spectra(spd_lib)

  Npeak <- list()
  for(i in 1:nrow(DuoResults$cor_spectra)){
    spd_sample <- DataFrame(msLevel = 2L,
                            id = DuoResults$cor_spectra$id[i])
    spd_sample$mz <- list(GcDuoObject$mz)
    spd_sample$intensity <- list(spec_mean_mat[i,])

    sps <- Spectra(spd_sample)
  ...
  if(!is.null(RI)){
    ...
    if(foreach::getDoParRegistered()){
      lib_peaks <- foreach::foreach(Peak2ID = Npeak, .options.snow = opts) %dopar% libIDs(Peak2ID,
                                                                                         lib_matrix,
                                                                                         libs_ri,
                                                                                         samples_ri)
    }
...
libIDs <- function(Peak2ID, lib_matrix, lib_ri, samples_ri = NULL) {
    require(Spectra)

    cor_cut <- NULL
    for(s in 1:nrow(lib_matrix)) {
      spd_lib <- DataFrame(msLevel = 2L,
                           id = row.names(lib_matrix)[s])
      spd_lib$mz <- list(as.numeric(colnames(lib_matrix)))
      spd_lib$intensity <- list(lib_matrix[s,])

      sps_lib <- Spectra(spd_lib)

      cor_cut[s] <- Spectra::compareSpectra(Peak2ID$sps, sps_lib, ppm = 5)
    }
  ...
  if(any(cor_cut >= Peak2ID$match_cutoff, na.rm = T)) {
    ...
    id_table <- data.frame("compound" = row.names(lib_matrix)[which(cor_cut > 0.7)],
                           "sim" = cor_cut[which(cor_cut > 0.7)])
    ...
    if(!is.null(Peak2ID$RI)){
      ...
      ris <- sapply(id_table$compound, function(x){
        a <- which(libs_ri$comp == x)
        return(libs_ri$ri[a[1]])})

      id_table$t_ri <- as.numeric(ris)

      # Get the library ID with the closest RI to the sample
      e_ri <- samples_ri[Peak2ID$pos]

      id_table.ri <- id_table[id_table$t_ri < e_ri + RIrange &
                                  id_table$t_ri > e_ri - RIrange,]
    ...
    if(!rlang::is_empty(pos)) {
      process <- data.frame("Name" = id_table$compound[pos],
                            "id" =  Peak2ID$id,
                            "RT1" = Peak2ID$time1D,
                            "RT2" = Peak2ID$time2D,
                            "sim" = id_table$sim[pos]
      )
    } else {
